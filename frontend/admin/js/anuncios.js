...
async function loadAnuncio(){
    ...
    const dataDenuncias = await apiClient.get(`/api/admin/anuncios/${anuncioId}/denuncias`);
    fillAnuncioInfo(data);
    fillDenuncias(dataDenuncias);
}
...
function fillDenuncias(data){
    const containerDenuncias = document.querySelector("#container-denuncias");
    containerDenuncias.innerHTML = data.map(denuncia => {
        return `<div class="card-recurso card-denuncia d-flex flex-column">
                    <p>${denuncia.motivo}</p>
                </div>`
    }).join('');
}
